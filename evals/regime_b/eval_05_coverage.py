"""
Eval 05: Frequentist Coverage (Regime B)

Goal: Verify 95% CI coverage for linear model with AnalyticLambda.

Procedure:
    For m = 1, ..., M:
        1. Generate Linear DGP data (confounded T)
        2. Run inference() with model="linear"
...
def run_single_sim(
    ...
    try:
        ...
        Y, T, X, theta_true, _ = generate_linear_data(n=n, seed=sim_id, dgp=dgp)
...
def run_eval_05_coverage(M: int = 20, n: int = 500, verbose: bool = True):
    """
    Run frequentist coverage evaluation for Regime B.
    """
    print("=" * 60)
    print("EVAL 05: FREQUENTIST COVERAGE (Regime B)")
    print("=" * 60)

    dgp = LinearDGP()
    mu_true = dgp.mu_true()

    print(f"\nDGP: Linear with confounded treatment")
    print(f"  α*(x) = x²")
    print(f"  β*(x) = 2x - 1")
...
if __name__ == "__main__":
    result = run_eval_05_coverage(M=20, n=500, verbose=True)
