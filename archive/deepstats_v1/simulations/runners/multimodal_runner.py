"""Multimodal (image+text) simulation runner."""
...
from simulations.dgp import get_multimodal_dgp


class MultimodalSimulationRunner(BaseSimulationRunner):
    """Simulation runner for multimodal (image+text) data.

    Runs DeepHTE with combined image and text features.
    Note: Comparison methods require extracted features from both modalities.
    """
    ...
    def _run_deephte(self, data: dict, seed: int) -> dict:
        """Run DeepHTE on combined features."""
        return self._run_with_features("deephte", data, seed)

    def _run_with_features(
        self, method: str, data: dict, seed: int
    ) -> dict:
        """Run method using combined image and text features."""
